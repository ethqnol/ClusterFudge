    ...
    def fit(self, X:npt.NDArray[np.float64]) -> None:
        ...
        for i in range(self.max_iter): #for the number of iterations, fit, then adjust
            ...
            for centroid in range(self.n_clusters):
                ...
                centroid_object = []
                for i, var in enumerate(points_in_centroid.items()): #can iterate over the index and the value (index is assigned to i, value is assigned to var)
                    centroid_object[i] = var.mode()
                self.centroids[centroid] = centroid_object #for each var, the item at centroids for 
